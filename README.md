...
- Latest version of MySQL Workbench and MySQL Server
- A text editor (like atom)
- Node.js
- A Windows machine <span style="color:red">(*strongly recommended*)</span>
...
- Install [Node.js](https://nodejs.org/en/download/) and React Native (type `npm install react-native`). You also need to follow [this guide](https://reactnative.dev/docs/environment-setup) to help walk you through setting up Android Studio and React Native. Specifically, you want to follow the *React Native CLI Quickstart* and *Android* options, which is what WE focused on). Next get the MySQL driver for Node.js. You can get it by typing `npm install mysql` in your command prompt.

- Clone the repository into a preferred directory.

- Fetch the necessary packages; go to the `CardsAgainstProfanityMobile` directory, and run `npm install`, which will install all dependencies listed in the `package.json` file (some of those should already be installed per our steps above, but others aren't yet installed in this directory). Go to the `server` directory, and do the same.
...
- - Enable developer mode, and USB debugging, following: https://developer.android.com/studio/debug/dev-options. Once you have done this, if you plug your phone into your desktop via USB, your Android should say in its notifications that it is connected to your laptop for USB debugging. Since you should have Android Studio installed, you can go into your shell and enter `adb devices`, and it should verify that your device is connected!
...
- - - `Join Lobby/index.js` line 48
- - - `Landing/deck-select.js` line 80
- - - `Lobby/index.js` line 52
- - - `Login/index.js` line 52
- - - `Scoreboard/index.js` line 47, 92
- - - `Stats/index.js` line 48
 
<span style="color:red">Please make sure to verify that nothing else has been overwritten as this will break the code.</span>

- - Do the same for the `postRequestCurl` file in the `Additional Project Information` directory.
- - Also, update `server/index.js` in the `server` directory with database information matching * *your* * MySQL instance (this involves changing line 55).


## Deployment/Running the Application
- Open three shell windows, all in the base folder where you cloned your repository.
- In the first shell window, go to the `server` directory, and run the command `node index.js`. You should see the output "Server listening on port 3000." in your shell.
